...
export const MEAL_RATE_PAISA = 5_200;
export const HALF_MEAL_COST_PAISA = MEAL_RATE_PAISA / 2;
...
export function calculateMonth(monthKey, entries = {}, advancePaisa = 0) {
  ...
  for (let day = 1; day <= count; day += 1) {
    ...
    daily.push({
      ...
      costPaisa: actualForDay * HALF_MEAL_COST_PAISA,
    });
  }

  const spentPaisa = actualHalfUnits * HALF_MEAL_COST_PAISA;
  ...
  return {
    ...
    specialExtraCostPaisa: specialExtraHalfUnits * HALF_MEAL_COST_PAISA,
...
export function buildMonthCsv(summary) {
  ...
  rows.push(['Meal rate (BDT)', formatMoney(MEAL_RATE_PAISA, { symbol: false, forceDecimals: true })]);
