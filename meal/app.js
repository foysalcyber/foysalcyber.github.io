...
function renderInsights() {
  ...
  $('#insightMaximumCost').textContent = `${formatMoney(summary.maximumHalfUnits * 2_600)} if every slot is eaten`;
...
async function saveProfile(event) {
  ...
  try {
    if (state.mode === 'demo') {
      state.profile = { displayName: name, email: 'Local preview' };
      ...
      state.profile = { displayName: name, email: state.user.email || '' };
    }
    updateUserInterface();
...
async function enterDemo() {
  ...
  state.profile = profile || { displayName: 'Demo Resident', email: 'Local preview' };
  state.summary = null;
  await showApp();
}
...
async function handleSignUp(event) {
  ...
  try {
    ...
    state.profile = { displayName: name, email };
...
function attachEventListeners() {
  ...
  window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') {
      ...
      else if (openModalElement) closeModal(openModalElement.id);
      $('#accountPopover').classList.add('is-hidden');
      closeSidebar();
    }
