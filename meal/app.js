...
const ANNOUNCEMENT_TYPES = ['info', 'warning', 'success'];
const DEMO_ANNOUNCEMENT = {
  title: 'Friday lunch counted as 1 meal',
  message: 'শুক্রবার দুপুরের খাবার সাধারণত ২ মিল ধরা হয়। কিন্তু আজকের (শুক্রবার) দুপুরের খাবার ১ মিল হিসেবে হিসাব করা হয়েছে। তাই সবাই দয়া করে নিজের হিসাব থেকে আরও একটি মিল কেটে দিন — তাহলে মাসের হিসাব ব্যালেন্স হয়ে যাবে।',
  type: 'warning',
  enabled: true,
};
...
async function showApp() {
  ...
  if (state.mode === 'firebase') {
    ...
    state.announcement = { ...DEMO_ANNOUNCEMENT, updatedAt: null };
    updateAnnouncementUI();
...
function sanitizeAnnouncement(data) {
  ...
  const message = typeof data?.message === 'string' ? data.message.trim().slice(0, 800) : '';
  const type = ANNOUNCEMENT_TYPES.includes(data?.type) ? data.type : 'info';
  const enabled = data?.enabled === true;
  return { title, message, type, enabled, updatedAt: data?.updatedAt ?? null };
}

function announcementIsActive() {
  const announcement = state.announcement;
  return Boolean(announcement && announcement.enabled && (announcement.message || announcement.title));
}

function announcementSignature(announcement) {
  const stamp = announcement?.updatedAt ? timestampMillis(announcement.updatedAt) : 'static';
  return `${announcement?.title ?? ''}||${announcement?.message ?? ''}||${stamp}`;
}

function announcementSeenKey() {
  return `meal-ledger:announcement:${state.user?.uid ?? 'anonymous'}`;
}

function startAnnouncementSubscription() {
  if (state.mode !== 'firebase' || !state.firebaseReady || !state.db) return;
  if (state.announcementUnsubscribe) return;
  const { doc, onSnapshot } = state.firebase.firestoreModule;
  state.announcementUnsubscribe = onSnapshot(doc(state.db, 'announcements', 'broadcast'), (snapshot) => {
    state.announcement = snapshot.exists() ? sanitizeAnnouncement(snapshot.data()) : null;
    updateAnnouncementUI();
    maybeShowAnnouncement();
    if (state.activeView === 'admin') renderAnnouncementEditor();
  }, () => {
    state.announcement = null;
    updateAnnouncementUI();
  });
...
function stopAnnouncementSubscription() {
  if (state.announcementUnsubscribe) {
    try { state.announcementUnsubscribe(); } catch { /* Listener already closed. */ }
  }
  state.announcementUnsubscribe = null;
}

function updateAnnouncementUI() {
  const bell = $('#announcementBellButton');
  const active = announcementIsActive();
  bell?.classList.toggle('is-hidden', !active);
  if (!active) return;
  // "Seen" is persisted per user in localStorage, so the pop-up appears only
  // once per message (per browser). The bell stays available to reopen it.
  const unread = localStorage.getItem(announcementSeenKey()) !== announcementSignature(state.announcement);
...
function renderAnnouncementModal() {
  ...
  const card = $('#announcementModal');
  card.classList.remove('is-info', 'is-warning', 'is-success');
  card.classList.add(`is-${announcement.type}`);
  $('#announcementModalTitle').textContent = announcement.title || 'Hostel notice';
  $('#announcementModalMessage').textContent = announcement.message || '';
}

function openAnnouncementModal() {
  if (!announcementIsActive()) return;
  renderAnnouncementModal();
  openModal('announcementModal');
}

function maybeShowAnnouncement() {
  if (!announcementIsActive() || !state.appActive) return;
  // localStorage (not sessionStorage) means each user sees the pop-up exactly
  // once per announcement version. A fresh message (or an edited one) gets a
  // new signature and will be shown once again.
  if (localStorage.getItem(announcementSeenKey()) === announcementSignature(state.announcement)) return;
...
function dismissAnnouncement() {
  ...
  if (announcementIsActive()) {
    localStorage.setItem(announcementSeenKey(), announcementSignature(state.announcement));
  }
  updateAnnouncementUI();
}
...
function renderAnnouncementEditor() {
  const announcement = state.announcement;
  const titleInput = $('#adminAnnouncementTitleInput');
  const messageInput = $('#adminAnnouncementMessageInput');
  const typeSelect = $('#adminAnnouncementTypeSelect');
  const enabledToggle = $('#adminAnnouncementEnabledToggle');
  if (!titleInput || !messageInput || !typeSelect || !enabledToggle) return;
  // Never overwrite a field the administrator is actively typing in.
  if (document.activeElement !== titleInput) titleInput.value = announcement?.title ?? '';
  if (document.activeElement !== messageInput) messageInput.value = announcement?.message ?? '';
  typeSelect.value = ANNOUNCEMENT_TYPES.includes(announcement?.type) ? announcement.type : 'info';
  enabledToggle.checked = announcement?.enabled === true;
  renderAnnouncementEditorStatus();
  updateAdminAnnouncementPreview();
}

function renderAnnouncementEditorStatus() {
  const badge = $('#adminAnnouncementStatusBadge');
  const toggle = $('#adminAnnouncementEnabledToggle');
  const live = toggle?.checked === true;
  toggle?.closest('.announcement-switch')?.classList.toggle('is-checked', live);
  if (!badge) return;
  badge.classList.toggle('is-live', live);
  badge.classList.toggle('is-paused', !live);
  $('span', badge).textContent = live ? 'LIVE' : 'PAUSED';
}

function updateAdminAnnouncementPreview() {
  const title = $('#adminAnnouncementTitleInput')?.value.trim() ?? '';
  const message = $('#adminAnnouncementMessageInput')?.value ?? '';
  const type = $('#adminAnnouncementTypeSelect')?.value ?? 'info';
  const card = $('#adminAnnouncementPreview');
  if (!card) return;
  card.classList.remove('is-info', 'is-warning', 'is-success');
  card.classList.add(`is-${ANNOUNCEMENT_TYPES.includes(type) ? type : 'info'}`);
  $('#adminAnnouncementPreviewTitle').textContent = title || 'Untitled notice';
  $('#adminAnnouncementPreviewMessage').textContent = message || 'Your notice text will appear here exactly as residents will see it.';
}

async function publishAnnouncement() {
  if (!isAdminUser() || !state.firebaseReady || !state.db) return;
  const title = $('#adminAnnouncementTitleInput').value.trim().slice(0, 80);
  const message = $('#adminAnnouncementMessageInput').value.trim().slice(0, 800);
  const type = $('#adminAnnouncementTypeSelect').value;
  const enabled = $('#adminAnnouncementEnabledToggle').checked;
  if (!title && !message) {
    toast('Nothing to publish', 'Write a title or message first.', 'error');
    return;
  }
  const button = $('#adminAnnouncementPublishButton');
  const { doc, setDoc, serverTimestamp } = state.firebase.firestoreModule;
  setButtonBusy(button, true, 'Publishing…');
  try {
    await setDoc(doc(state.db, 'announcements', 'broadcast'), {
      title,
      message,
      type: ANNOUNCEMENT_TYPES.includes(type) ? type : 'info',
      enabled,
      updatedAt: serverTimestamp(),
      updatedBy: state.user.email || ADMIN_EMAIL,
    }, { merge: true });
    toast('Announcement published', enabled
      ? 'The notice is now live and will pop up for every resident.'
      : 'Saved. The notice is paused and hidden from residents.');
  } catch (error) {
    toast('Could not publish announcement', humanFirebaseError(error), 'error');
  } finally {
    setButtonBusy(button, false);
  }
}
...
async function disableAnnouncement() {
  if (!isAdminUser() || !state.firebaseReady || !state.db) return;
  const button = $('#adminAnnouncementDisableButton');
  const { doc, setDoc, serverTimestamp } = state.firebase.firestoreModule;
  ...
  try {
    ...
    $('#adminAnnouncementEnabledToggle').checked = false;
    renderAnnouncementEditorStatus();
    toast('Announcement turned off', 'The notice is paused. Residents will no longer see it.');
  } catch (error) {
    toast('Could not turn off announcement', humanFirebaseError(error), 'error');
  } finally {
    setButtonBusy(button, false);
  }
}
...
function attachEventListeners() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const dismissAnnouncementEl = event.target.closest('[data-dismiss-announcement]');
    if (dismissAnnouncementEl) dismissAnnouncement();
  ...
  $('#announcementBellButton').addEventListener('click', openAnnouncementModal);
  $('#announcementGotItButton').addEventListener('click', dismissAnnouncement);
  $('#adminAnnouncementForm').addEventListener('submit', (event) => {
    event.preventDefault();
    publishAnnouncement();
  });
  $('#adminAnnouncementDisableButton').addEventListener('click', disableAnnouncement);
  $('#adminAnnouncementTitleInput').addEventListener('input', updateAdminAnnouncementPreview);
  $('#adminAnnouncementMessageInput').addEventListener('input', updateAdminAnnouncementPreview);
  $('#adminAnnouncementTypeSelect').addEventListener('change', updateAdminAnnouncementPreview);
  $('#adminAnnouncementEnabledToggle').addEventListener('change', renderAnnouncementEditorStatus);
